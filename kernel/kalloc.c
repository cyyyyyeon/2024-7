...
#define PA2PGREF_ID(p) (((p)-KERNBASE)/PGSIZE)//将物理地址转换为对应的引用计数字段的索引
#define PGREF_MAX_ENTRIES PA2PGREF_ID(PHYSTOP)//定义了引用计数数组的大小
...
//如果引用计数大于1，表示该页被多个进程共享，
//它会创建一个新的物理页并将数据复制到新页中，
//同时减少原页的引用计数。
