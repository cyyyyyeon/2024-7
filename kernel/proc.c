...
exit(int status)
{
  ...
  for (i = 0; i < NVMA; ++i) {
    ...
    vma = &p->vma[i];
// 检查 VMA 是否有 MAP_SHARED 标志。如果有，则需要将脏页写回文件。
// 遍历 VMA 的每个页面地址 va。
// 如果页面不是脏页，跳过。
// 计算页面中需要写回的字节数 n。
// 将页面分块写回文件，每次写入 n1 字节。
// 调用 begin_op 和 end_op 函数进行文件系统操作的开始和结束，确保文件系统的原子性。
// 使用 ilock 和 iunlock 锁定和解锁文件 vma->f->ip。
// 使用 writei 函数将数据写入文件。如果写入失败，触发恐慌。
